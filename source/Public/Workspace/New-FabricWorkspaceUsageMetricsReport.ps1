...
Author: Ioana Bouariu
    ...
    if ($PSCmdlet.ShouldProcess("Workspace Usage Metrics Report", "Retrieve")) {
        $headerParams = Get-PSFConfigValue FabricTools.FabricSession.Headers
        $data = Invoke-WebRequest -Uri "$url/$workspaceId/usageMetricsReportV2?experience=power-bi" -Headers $headerParams -ErrorAction SilentlyContinue
