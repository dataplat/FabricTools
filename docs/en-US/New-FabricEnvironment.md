...
```
New-FabricEnvironment [-WorkspaceId] <guid> [-EnvironmentName] <string>
 [[-EnvironmentDescription] <string>] [-WhatIf] [-Confirm] [<CommonParameters>]
```
...
The `Add-FabricEnvironment` function creates a new environment within a given workspace by making a POST request to the Fabric API.
The environment can optionally include a description.
...
(Mandatory) The name of the environment to be created.
