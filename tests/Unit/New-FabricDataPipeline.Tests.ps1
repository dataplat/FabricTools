...
BeforeDiscovery {
    $CommandName = 'New-FabricDataPipeline'
}
...
Describe "New-FabricDataPipeline" -Tag "UnitTests" {
    ...
    Context 'Command definition' {
        ...
        It 'Should have the expected parameter: <Name>' -ForEach @(
            ...
            @{ Name = 'DataPipelineName'; Mandatory = $true }
            @{ Name = 'DataPipelineDescription'; Mandatory = $false }
        ) {
    ...
    Context 'When creating data pipeline successfully with immediate completion (201)' {
        BeforeAll {
        ...
        It 'Should return the created data pipeline' {
            $mockWorkspaceId = [guid]::NewGuid()

            $result = New-FabricDataPipeline -WorkspaceId $mockWorkspaceId -DataPipelineName 'TestDataPipeline' -Confirm:$false

            $result.displayName | Should -Be 'TestDataPipeline'
        }
    }
    ...
    Context 'When an unexpected status code is returned' -Skip {
        ...
        It 'Should write an error message for unexpected status codes' {
            ...
            { New-FabricDataPipeline -WorkspaceId $mockWorkspaceId -DataPipelineName 'TestDataPipeline' -Confirm:$false } | Should -Not -Throw
